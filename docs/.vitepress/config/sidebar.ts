...
export const sidebar: DefaultTheme.Config['sidebar'] = {
  ...
  '/courses/webgl-base/': getItems("courses/webgl-base", true),
}
 ...
 * @returns {DefaultTheme.SidebarGroup[]}
 */
function getItems (path: string, noCal: boolean) {
  // 侧边栏分组数组
  ...
  sync(`docs/${path}/*`, {
    ...
    if (items.length > 0) {
      ...
      groups.push({
        text: `${groupName.substring(groupName.indexOf('-') + 1)} ${noCal ? '' : '('+items.length+'篇)'} `,
        collapsible: true,
        collapsed: false,
        items: items
      })
